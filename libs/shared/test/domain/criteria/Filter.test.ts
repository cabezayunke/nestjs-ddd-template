...
describe('Filter', () => {
  const usernameFilterValues = {
    field: 'username',
    operator: Operator.EQUAL,
    value: 'yunke',
  };
  const emailFilterValues = {
    field: 'email',
    operator: Operator.NOT_EQUAL,
    value: 'yunke@gmail.com',
  };
  ...
  test('should throw error for missing field', () => {
    ...
    // act
    // assert
    expect(() => SingleFilter.fromPrimitives(opAndValue as FilterPrimitives)).toThrow(
  ...
  test('should throw error for missing operator', () => {
    ...
    // act
    // assert
    expect(() => SingleFilter.fromPrimitives(fieldAndValue as FilterPrimitives)).toThrow(
      'The filter is invalid',
    );
  });
  ...
  test('should throw error for missing value', () => {
    ...
    // act
    // assert
    expect(() => SingleFilter.fromPrimitives(fieldAndOp as FilterPrimitives)).toThrow(
      'The filter is invalid',
    );
  });
  ...
  test('should throw error for multifilter with less than 2 filters', () => {
    ...
    // assert
    expect(() =>
      MultiFilter.and(SingleFilter.fromPrimitives(usernameFilterValues)),
    ).toThrow('You need at least 2 filters');
  });
