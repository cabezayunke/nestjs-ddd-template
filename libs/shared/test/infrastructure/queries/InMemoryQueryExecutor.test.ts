...
describe('InMemoryQueryExecutor', () => {
  ...
  test.each([
    {
      operator: 'EQUAL',
      criteria: Criteria.equal('id', 'myId'),
      expectedResult: [item1],
    },
    {
      operator: 'NOT_EQUAL',
      criteria: Criteria.notEqual('id', 'myId'),
      expectedResult: [item2, item3],
    },
    {
      operator: 'CONTAINS',
      criteria: Criteria.contains('id', 'my'),
      expectedResult: [item1, item2],
    },
    {
      operator: 'NOT_CONTAINS',
      criteria: Criteria.notContains('id', 'my'),
      expectedResult: [item3],
    },
    {
      operator: 'IN',
      criteria: Criteria.in('id', ['myId', 'myId2']),
      expectedResult: [item1, item2],
    },
    {
      operator: 'NOT_IN',
      criteria: Criteria.notIn('id', ['myId', 'myId2']),
      expectedResult: [item3],
    },
  ])(
    'should handle single $operator criteria with results',
  ...
  test.each([
    ...
    {
      operator: 'NOT_IN',
      criteria: Criteria.notIn('id', ['myId', 'myId2', 'anotherId']),
    },
  ...
  describe('Pagination', () => {
    test.each([
      ...
      'should get paginated results for limit $criteria.pagination.limit.value, offset $criteria.pagination.offset.value',
      ({ criteria, expectedResult }) => {
        // arrange
        // act
        const result = executor.execute<CustomResponse>(criteria) as CustomResponse[];
